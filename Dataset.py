...
def cut_patches(image, patches):
    ...
    left_eye = transforms.functional.pad(
        left_eye, (0, 0, 48 - left_eye.size[-2], 24 - left_eye.size[-1])
    )
    right_eye = transforms.functional.crop(
        image, *process_coord(**patches["Right eye"])
    )
    right_eye = transforms.functional.pad(
        right_eye, (0, 0, 48 - right_eye.size[-2], 24 - right_eye.size[-1])
    )
    nose = transforms.functional.crop(image, *process_coord(**patches["Nose"]))
    nose = transforms.functional.pad(
        nose, (0, 0, 48 - nose.size[-2], 64 - nose.size[-1])
    )
    mouth = transforms.functional.crop(image, *process_coord(**patches["Mouth"]))
    mouth = transforms.functional.pad(
        mouth, (0, 0, 80 - mouth.size[-2], 32 - mouth.size[-1])
    )
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        batch["left_eye"], batch["right_eye"], batch["nose"], batch["mouth"] = (
            cut_patches(image, image_info)
        )
        batch["left_eyeGT"], batch["right_eyeGT"], batch["noseGT"], batch["mouthGT"] = (
            cut_patches(imageGT, self.images_list[image_info["imgGT"]])
        )
        batch["patches"] = {
            "left_eye": image_info["Left eye"],
            "right_eye": image_info["Right eye"],
            "mouth": image_info["Mouth"],
            "nose": image_info["Nose"],
        }
...
def createDataset(images_list, images_dir, p_test=0.1):
    with open(images_list, "r") as rf:
        ...
        # print("files loaded from yaml", images_list_all)
    ...
    return CustomDataset(images_list_train, images_list_all, images_dir), CustomDataset(
        images_list_test, images_list_all, images_dir
    )
